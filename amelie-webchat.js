...
(function (global) {
  ...
  var ICON_CLOSE = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg>';
  var ICON_SEND =
  ...
  function createWidget(config) {
    ...
    var posMap = { 'bottom-right': 'br', 'bottom-left': 'bl', 'top-left': 'tl', 'top-right': 'tr' };
    root.className = 'amelie-wc-root amelie-wc-pos-' + (posMap[position] || 'br');
    ...
    function sendMessage(text, messagesEl, inputEl) {
      ...
      var mid = 'webchat-' + Date.now() + '-' + Math.random().toString(36).slice(2);
      var payload = {
        botId: botId,
        pregunta: text.trim(),
        senderId: botId,
      ...
      fetch(askUrl, {
        ...
        .then(function (res) {
          if (!res.ok) return res.json().then(function (j) { throw new Error(j.message || 'Error ' + res.status); });
          return res.json();
        })
        .then(function (data) {
          ...
          messages.push({
            role: 'assistant',
            content: (data && data.content && data.content.trim()) || '(Sin respuesta)',
          });
          renderMessages(messagesEl);
        ...
        .catch(function (err) {
          ...
          messages.push({
            role: 'assistant',
            content: err && err.message ? err.message : 'Error al enviar el mensaje.',
          });
          renderMessages(messagesEl);
          messagesEl.scrollTop = messagesEl.scrollHeight;
        })
    ...
    return {
      open: function () { panel.classList.add('open'); input.focus(); },
